<template>
  <div class="max-w-6xl mx-auto">
    <h1 class=" text-2xl font-bold mb-3 text-indigo-500 " >
      Jobs of #{{route.params.slug}}
    </h1>
...
const { stop } = useIntersectionObserver(
  endOfJobs, async ([{isIntersecting}]) => {
    if(isIntersecting && (pagination.current_page < pagination.last_page)){
      ...
      const {ok, data } = await api.get(`api/tags/${route.params.slug}/jobs?page=${++pagination.current_page}`);
      if(ok){
        pagination.current_page = data.meta.current_page;
        pagination.last_page = data.meta.last_page;
        console.log(data);
